...
class TestSettings:
    ...
    def test_settings_loads_with_defaults(self):
        ...
        with patch.dict(os.environ, {"POSTGRES_URL": "postgresql://test:test@localhost:5432/test"}, clear=True):
    ...
    def test_settings_production_mode(self):
        ...
        with patch.dict(os.environ, {"STAGE": "prod", "POSTGRES_URL": "postgresql://prod:prod@prod:5432/prod"}, clear=True):
            settings = Settings(_env_file=None)
    ...
    def test_aws_detection(self):
        ...
        with patch.dict(
            os.environ,
            {"POSTGRES_URL": "postgresql://test:test@localhost:5432/test", "AWS_EXECUTION_ENV": "AWS_Lambda_python3.11"},
            clear=True,
        ):
            settings = Settings(_env_file=None)
