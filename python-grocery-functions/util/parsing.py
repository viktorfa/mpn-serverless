...
from util.constants import quantity_units, si_mappings, piece_units, piece_value_units, quantity_value_units
...
def extract_number(string: str, loader_context: dict = dict()) -> Optional[float]:
    try:
        ...
        logging.warning(
            '' + str(exc) + 'Could not extract number from: ' + str(loader_context.items()))
        return None
