...
def get_spider_config_by_id(config_id: str):
    ...
    with Session(pg_engine) as session:
        try:
            result = session.query(SpiderConfigsTable).filter(SpiderConfigsTable.id == config_id).one_or_none()
            return result
...
def get_spider_config_by_mongo_id(mongo_id: str):
    ...
    with Session(pg_engine) as session:
        try:
            result = session.query(SpiderConfigsTable).filter(SpiderConfigsTable.mongo_id == mongo_id).one_or_none()
            return result
        except Exception as e:
            print(f"An error occurred: {e}")
            raise e
